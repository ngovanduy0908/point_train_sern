...
function joinArrayWithComma(arr) {
  if (!Array.isArray(arr)) {
    return ""; // Trả về chuỗi rỗng nếu đầu vào không phải là mảng
  }
...
export const viewExcelBC = async (req, res) => {
  ...
  const arrKhoaHoc = danhSachKhoaHoc?.map((item) => item.maKhoaHoc);
  const arrTenKhoaHoc = danhSachKhoaHoc?.map((item) => item.tenKhoaHoc);

  // console.log("arr khoa hoc: ", arrKhoaHoc);

  const convertArrKhoaHoc = joinArrayWithComma(arrKhoaHoc);
  const convertArrTenKhoaHoc = joinArrayWithComma(arrTenKhoaHoc);

  // console.log("arr join: ", convertArrTenKhoaHoc);
  ...
  point.status_teacher = 1`;
    ...
    class.maKhoaHoc IN (${convertArrKhoaHoc}) and
    point.maHK = '${maHK.maHK}' and
    point_teacher.maHK = '${maHK.maHK}' and
    point.point_teacher >= '${danhSachSV.min}' and point.point_teacher <= '${danhSachSV.max}' and
    point.status_teacher = 1
    ...
    db.query(q, (err, data) => {
      ...
      const values = {
        ...
        tenVaKhoa: `${convertToUpperCase(
          maCN?.tenCN
        )} - KHOA: ${convertToUpperCase(tenKhoa)} - ${convertArrTenKhoaHoc}`,
        tenLopTruong: tenLopTruong ? tenLopTruong : "",
        day: day,
        month: month,
        year: year,
    ...
    if (isToanKhoaHoc === 1) {
      ...
      point_teacher.maHK = '${maHK.maHK}' and
      point.point_teacher >= '${danhSachSV.min}' and point.point_teacher <= '${danhSachSV.max}' and
      point.status_teacher = 1 and class.maKhoa = '${maKhoa}';
      `;
    } else {
      q = `
      select
      ...
      point.point_teacher >= '${danhSachSV.min}' and point.point_teacher <= '${danhSachSV.max}' and
      point.status_teacher = 1 and class.maKhoa = '${maKhoa}'
      and class.maKhoaHoc in (${arrKhoaHoc})
      `;
    }
    // console.log("xuong tan day nay");
    db.query(q, (err, data) => {
      if (err) return res.status(500).json(err);
      const handleData = data?.map((item, idx) => ({
        ...item,
      ...
      const values = {
        ...
        hk: maHK ? convertToUpperCase(maHK?.tenHK) : "",
        tenKhoa: convertToUpperCase(tenKhoa),
        tenVaKhoa: `KHOA: ${convertToUpperCase(tenKhoa)} - ${
          isToanKhoaHoc === 1 ? "" : convertArrTenKhoaHoc
        }`,
        tenLopTruong: tenLopTruong ? tenLopTruong : "",
        day: day,
        month: month,
